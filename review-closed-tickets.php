          ...
          <div class="count">
            ...
            <?php
              if ($_SESSION['user_type'] == "Administrator"){
                ...
                $query = "SELECT (*) as count FROM user_access_ticket_t uat LEFT JOIN ticket_t t USING(ticked_id) WHERE t.ticket_status=8";
              }
              else if ($_SESSION['user_type'] == "Technicals Group Manager"){
                $query = "SELECT COUNT(*) as count  FROM service_ticket_t st LEFT JOIN ticket_t t USING(ticket_id) WHERE t.ticket_status=8";
              }
              else if ($_SESSION['user_type'] == "Network Group Manager"){
                $query = "SELECT COUNT(*) as count FROM ticket_t WHERE ticket_status=8 AND ticket_category='Network'";
              }
              else if ($_SESSION['user_type'] == "Technician"){
                $query = "SELECT COUNT(*) as count FROM service_ticket_t st LEFT JOIN ticket_t t USING(ticket_id) WHERE t.ticket_status=8 AND t.ticket_agent_id = '".$_SESSION['user_id']."'";
              }
              else if ($_SESSION['user_type'] == "Network Engineer"){
                $query = "SELECT COUNT(*) as count FROM user_access_ticket_t uat LEFT JOIN ticket_t t USING(ticket_id) WHERE t.ticket_status=8 AND t.ticket_agent_id = '".$_SESSION['user_id']."'";
              }
